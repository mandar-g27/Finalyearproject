...
// Determine what speech + description to show based on which step failed
function getDeniedReason(failedStep) {
  switch (failedStep) {
    ...
    default:
      return {
        desc: "Biometric verification could not be completed. Please contact your system administrator.",
        speech: "Access denied. Biometric verification failed.",
      };
  }
}

// Per-step status: "pass" | "fail" | "skip"
    ...
    <div className={`result-screen ${granted ? "granted" : "denied"} ${visible ? "visible" : ""}`}>
      ...
      <div className="result-content">
        ...
        <h1 className="result-title">
          {granted ? "Welcome Back" : "Verification Failed"}
        </h1>

        <p className="result-desc">{desc}</p>

        {/* Per-step status breakdown */}
        ...
        <div className="result-meta">
          {sessionId && <span>Session: #{sessionId.slice(0, 10).toUpperCase()}</span>}
          <span>{timestamp}</span>
        </div>
      ...
      <style>{`
        ...
        .icon-ring.outer {
          width: 140px; height: 140px;
          border: 1px solid;
          animation: ringExpand 0.6s ease 0.4s both;
        }
        .icon-ring.inner {
          width: 110px; height: 110px;
          border: 1.5px solid;
          animation: ringExpand 0.6s ease 0.6s both;
        }
        ...
        .result-desc {
          font-size: 14px; color: rgba(180,200,220,0.6);
        ...
        .result-meta {
          ...
          font-size: 11px; color: rgba(150,170,190,0.35);
